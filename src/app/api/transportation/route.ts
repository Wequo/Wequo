...
import {getCompanyScheme} from "@/utils/services/transportation/getCompanyScheme";
...
export async function POST(request: Request) {
  ...
  const companyScheme = await getCompanyScheme(companyId);


  if (!companyScheme) {
    return NextResponse.json({ success: false, error: 'scheme no encontrado' }, { status: 400 });
  }

  console.log(companyScheme)
...
const needsVehicleExtraData = () => {
  ...
  const companyEmailBody = `
        ...
        <p style="margin: 0;"></p>
            <p style="margin: 0;">
                <span style="font-size: 15px;">
                    ...
                    ${formData.tripType !== 'oneWay' ? `- <strong>Fecha de Regreso:</strong> ${formatDates(formData.endDate)}` : ''}
                </span>
            </p>
            <p style="margin: 0;"></p>
            <p style="margin: 0;">
        ...
        <p style="margin: 0;"><span style="font-size: 15px;"><strong>Dirección de orígen</strong>: ${formData.departureAddress}</span></p><p style="margin: 0;"></p>
  ...
  const clientEmailBody = `<div>
    ...
    <p style="margin: 0;"></p>
    <p style="margin: 0;">
        <span style="font-size: 15px;"><strong>Dirección de destino</strong>: ${formData.arrivalAddress}</span>
    </p>
    <p style="margin: 0;"></p>
    <p style="margin: 0;">
